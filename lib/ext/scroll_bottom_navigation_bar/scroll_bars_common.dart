...
class Snap extends StatelessWidget {
  ...
  bool _onNotification(UserScrollNotification notification) {
    if (notification.direction == ScrollDirection.idle) {
      ...
      final offset = controller._heightFactor.round() == 1 ? -controller._delta : controller.height - controller._delta;
...
abstract class ScrollBarsController {
  ...
  void _scrollListener() {
    ...
    if (position.pixels > position.maxScrollExtent) {
      ...
      _oldOffset = position.maxScrollExtent;
      return;
    }

    if (position.axisDirection == AxisDirection.down && position.pixels > position.maxScrollExtent) {
